...
{
    [ServiceContract]
    class BLL_FRM_SiJiGuanLi_Update
    {
        DALPublic.DALMethod DAL = new DALPublic.DALMethod();
        [OperationContract]
        public DataSet FRM_SiJiGuanLi_Load_Update_ChaXunZhunJiaCheXin()
        {
        ...
        [OperationContract]
        public bool btnQueDing_Click_XiuGaiSiJi(string 司机名称, string 出生日期, string 联系电话, string 联系地址, string 身份证号, string 驾驶证号, string 备注
            , int 机构组织ID, int 准驾车型ID, bool 是否离职,int 司机ID)
        {
            SqlParameter[] pas = new SqlParameter[] {new SqlParameter("@Type",SqlDbType.Char),
                ...
                new SqlParameter("@准驾车型ID",SqlDbType.Int),new SqlParameter("@是否离职",SqlDbType.Bit),new SqlParameter("@司机ID",SqlDbType.Int)
            };
            pas[0].Value = "btnQueDing_Click_XiuGaiSiJi";
            pas[1].Value = 司机名称;
            pas[2].Value = 出生日期;
            pas[3].Value = 联系电话;
            pas[4].Value = 联系地址;
            ...
            pas[10].Value = 是否离职;
            pas[11].Value = 司机ID;
            if (DAL.UpdateData("司机管理_FRM_SiJiGuanLi_Update", pas) > 0)
            {
                return true;
            }
            else
